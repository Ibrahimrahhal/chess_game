...
export default class Zobrist {
  ...
  static Position = {
    ...
    EN_PASSANT_FILE:
      Zobrist.Count.TURN +
      Zobrist.Count.PIECE_COLOR_SQUARE +
      Zobrist.Count.CASTLING_RIGHTS,
  };
  ...
  static RANDOM_VALUES = Zobrist.createRandomValues(
    Zobrist.Position.EN_PASSANT_FILE + Zobrist.Count.EN_PASSANT_FILE
  );
  ...
  updatePieceColorSquare(piece, color, index) {
    return this.update(
      Zobrist.Position.PIECE_COLOR_SQUARE + piece + color * 6 + index * 6 * 2
    );
  }
