...
export default class Position {
  ...
  static SLIDING_MASKS = [
    Bitboard.makeFile(Chess.LAST_FILE).not(),
    Bitboard.ONE,
    Bitboard.makeFile(0).not(),
  ];
  static Status = {
    NORMAL: 0,
    CHECKMATE: 1,
  ...
  constructor() {
    ...
    this.bitboards = [
      ...
      Bitboard.makeIndex(1)
        .or(Bitboard.makeIndex(6))
        .or(Bitboard.makeIndex(57))
        .or(Bitboard.makeIndex(62)), // knights
      Bitboard.makeIndex(2)
        .or(Bitboard.makeIndex(5))
        .or(Bitboard.makeIndex(58))
        .or(Bitboard.makeIndex(61)), // bishops
      Bitboard.makeIndex(0)
        .or(Bitboard.makeIndex(7))
        .or(Bitboard.makeIndex(56))
        .or(Bitboard.makeIndex(63)), // rooks
  ...
  getMoves(pseudoLegal, onlyCaptures) {
    ...
    return pseudoLegal
      ? moves
      : moves.filter(Position.prototype.isMoveLegal, this);
  }
  ...
  getKingPosition(color) {
    return this.getPieceColorBitboard(
      Chess.Piece.KING,
      color
  ...
  getOccupiedBitboard() {
    return this.bitboards[Position.ANY_WHITE]
      .dup()
      .or(this.bitboards[Position.ANY_BLACK]);
  }
  ...
  updateHashKey() {
    ...
    for (
      var color = Chess.PieceColor.WHITE;
      color <= Chess.PieceColor.BLACK;
      ++color
    ) {
      for (var piece = Chess.Piece.PAWN; piece <= Chess.Piece.KING; ++piece) {
        this.hashKey.updatePieceColorBitboard(
          piece,
          color,
          this.getPieceColorBitboard(piece, color)
        );
      }
    }
  ...
  isKingInCheck() {
    return this.isAttacked(
      Chess.getOtherPieceColor(this.getTurnColor()),
      this.getKingPosition(this.getTurnColor())
    );
  }
  ...
  static makePawnAttackMask(color, pawns) {
    ...
    var attacks1 = pawns
      .dup()
      .and_not(Bitboard.FILES[0])
      .shiftLeft(white ? 7 : -9);
    var attacks2 = pawns
      .dup()
      .and_not(Bitboard.FILES[Chess.LAST_FILE])
      .shiftLeft(white ? 9 : -7);
  ...
  static makeSlidingAttackMask(fromBB, occupied, rankDirection, fileDirection) {
    ...
    for (
      fromBB.shiftLeft(direction);
      !fromBB.and(mask).isEmpty();
      fromBB.and_not(occupied).shiftLeft(direction)
    ) {
      bb.or(fromBB);
    }
  ...
  static makeBishopAttackMask(fromBB, occupied) {
    return Position.makeSlidingAttackMask(fromBB.dup(), occupied, 1, 1)
      .or(Position.makeSlidingAttackMask(fromBB.dup(), occupied, 1, -1))
      .or(Position.makeSlidingAttackMask(fromBB.dup(), occupied, -1, 1))
      .or(Position.makeSlidingAttackMask(fromBB.dup(), occupied, -1, -1));
  }

  static makeRookAttackMask(fromBB, occupied) {
    return Position.makeSlidingAttackMask(fromBB.dup(), occupied, 0, 1)
      .or(Position.makeSlidingAttackMask(fromBB.dup(), occupied, 0, -1))
      .or(Position.makeSlidingAttackMask(fromBB.dup(), occupied, 1, 0))
      .or(Position.makeSlidingAttackMask(fromBB.dup(), occupied, -1, 0));
  }
  ...
  canCastle(color, kingSide, onlyLegal) {
    ...
    if (
      occupied.isSet(kingPosition + direction) ||
      occupied.isSet(kingPosition + 2 * direction)
    ) {
      return false;
    }
  ...
  isCastlingLegal(color, kingSide) {
    ...
    return (
      !this.isAttacked(otherColor, kingPosition) &&
      !this.isAttacked(otherColor, kingPosition + direction) &&
      !this.isAttacked(otherColor, kingPosition + 2 * direction)
    );
  }
  ...
  isThreefoldRepetitionRuleDraw() {
    ...
    return (
      this.hashHistory.reduce(function (
        previousValue,
        currentValue,
        index,
        array
      ) {
        return previousValue + (currentValue.isEqual(currentHashKey) ? 1 : 0);
      },
      0) >= 3
    );
  }
  ...
  isInsufficientMaterialDraw() {
    ...
    if (
      bishops.dup().and(Bitboard.LIGHT_SQUARES).isEqual(bishops) ||
      bishops.dup().and(Bitboard.DARK_SQUARES).isEqual(bishops)
    ) {
      return true;
    }
  ...
  isDraw() {
    return (
      ...
      this.isThreefoldRepetitionRuleDraw() ||
      this.isInsufficientMaterialDraw()
    );
  }
  ...
  getStatus() {
    if (!this.getMoves().length) {
      return this.isKingInCheck()
        ? Position.Status.CHECKMATE
        : Position.Status.STALEMATE_DRAW;
    }
  ...
  generateMoves(onlyCaptures) {
      ...
      while (!toMask.isEmpty()) {
        ...
        moves.push(
          new Controller(
            ...
            kind,
            Chess.Piece.PAWN,
            chessPosition.getPieceAtOrNull(index)
          )
        );
      }
    }
    function addPawnPromotions(toMask, movement, capture) {
      addPawnMoves(
        ...
        movement,
        capture
          ? Controller.Kind.QUEEN_PROMOTION_CAPTURE
          : Controller.Kind.QUEEN_PROMOTION
      );
      addPawnMoves(
        toMask.dup(),
        movement,
        capture
          ? Controller.Kind.ROOK_PROMOTION_CAPTURE
          : Controller.Kind.ROOK_PROMOTION
      );
      addPawnMoves(
        toMask.dup(),
        movement,
        capture
          ? Controller.Kind.BISHOP_PROMOTION_CAPTURE
          : Controller.Kind.BISHOP_PROMOTION
      );
      addPawnMoves(
        toMask.dup(),
        movement,
        capture
          ? Controller.Kind.KNIGHT_PROMOTION_CAPTURE
          : Controller.Kind.KNIGHT_PROMOTION
      );
    }
    ...
    if (!onlyCaptures) {
      var doublePawnPushed = turnPawns
        .dup()
        .and(Bitboard.RANKS[turnColor ? 6 : 1])
        .shiftLeft(2 * rankDirection)
        .and_not(occupied)
        .and_not(occupied.dup().shiftLeft(rankDirection));
      addPawnMoves(
        doublePawnPushed,
        2 * rankDirection,
        Controller.Kind.DOUBLE_PAWN_PUSH
      );
      var positionalPawnMoved = turnPawns
        .dup()
        .shiftLeft(rankDirection)
        .and_not(occupied);
      addPawnMoves(
        positionalPawnMoved.dup().and_not(lastRow),
        rankDirection,
        Controller.Kind.POSITIONAL
      );
      addPawnPromotions(
        positionalPawnMoved.dup().and(lastRow),
        rankDirection,
        false
      );
    }
    ...
    var pawnLeftCaptured = turnPawns
      .dup()
      .and_not(leftFile)
      .shiftLeft(leftCaptureMovement)
      .and(opponentBB);
    addPawnMoves(
      pawnLeftCaptured.dup().and_not(lastRow),
      leftCaptureMovement,
      Controller.Kind.CAPTURE
    );
    addPawnPromotions(
      pawnLeftCaptured.dup().and(lastRow),
      leftCaptureMovement,
      true
    );
    ...
    var pawnRightCaptured = turnPawns
      .dup()
      .and_not(rightFile)
      .shiftLeft(rightCaptureMovement)
      .and(opponentBB);
    addPawnMoves(
      pawnRightCaptured.dup().and_not(lastRow),
      rightCaptureMovement,
      Controller.Kind.CAPTURE
    );
    addPawnPromotions(
      pawnRightCaptured.dup().and(lastRow),
      rightCaptureMovement,
      true
    );

    if (this.canEnPassant()) {
      var pawnLeftEnPassant = Bitboard.makeIndex(
        this.getEnPassantSquare() + fileDirection
      )
        .and(turnPawns)
        .and_not(leftFile)
        .shiftLeft(leftCaptureMovement);
      addPawnMoves(
        pawnLeftEnPassant,
        leftCaptureMovement,
        Controller.Kind.EN_PASSANT_CAPTURE
      );
      var pawnRightEnPassant = Bitboard.makeIndex(
        this.getEnPassantSquare() - fileDirection
      )
        .and(turnPawns)
        .and_not(rightFile)
        ...
        moves.push(
          new Controller(
            ...
            opponentBB.isSet(to)
              ...
              : Controller.Kind.POSITIONAL,
            piece,
            chessPosition.getPieceAtOrNull(to)
          )
        );
      }
    }
    ...
    var turnKnights = this.getPieceColorBitboard(
      Chess.Piece.KNIGHT,
      turnColor
    ).dup();
    while (!turnKnights.isEmpty()) {
      ...
      addNormalMoves(
        knightPosition,
        Bitboard.KNIGHT_MOVEMENTS[knightPosition].dup().and(mask),
        Chess.Piece.KNIGHT
      );
    }

    var turnQueens = this.getPieceColorBitboard(
      Chess.Piece.QUEEN,
      turnColor
    ).dup();
    while (!turnQueens.isEmpty()) {
      ...
      addNormalMoves(
        queenPosition,
        Position.makeBishopAttackMask(
          Bitboard.makeIndex(queenPosition),
          occupied
        )
          .or(
            Position.makeRookAttackMask(
              Bitboard.makeIndex(queenPosition),
              occupied
            )
          )
    ...
    var turnBishops = this.getPieceColorBitboard(
      Chess.Piece.BISHOP,
      turnColor
    ).dup();
    while (!turnBishops.isEmpty()) {
      ...
      addNormalMoves(
        bishopPosition,
        Position.makeBishopAttackMask(
          Bitboard.makeIndex(bishopPosition),
          occupied
        ).and(mask),
        Chess.Piece.BISHOP
      );
    ...
    var turnRooks = this.getPieceColorBitboard(
      Chess.Piece.ROOK,
      turnColor
    ).dup();
    while (!turnRooks.isEmpty()) {
      ...
      addNormalMoves(
        rookPosition,
        Position.makeRookAttackMask(
          Bitboard.makeIndex(rookPosition),
          occupied
        ).and(mask),
        Chess.Piece.ROOK
      );
    ...
    addNormalMoves(
      kingPosition,
      Bitboard.KING_MOVEMENTS[kingPosition].dup().and(mask),
      Chess.Piece.KING
    );
  ...
  makeMove(move) {
    ...
    if (move.getPiece() === Chess.Piece.KING) {
      ...
      if (move.getFrom() === Position.getCastlingRookSquare(turnColor, true)) {
        ...
        move.getFrom() === Position.getCastlingRookSquare(turnColor, false)
      ) {
        this.clearCastlingRight(turnColor, false);
      }
    }
    ...
    if (move.getCapturedPiece() === Chess.Piece.ROOK) {
      if (
        move.getCaptureSquare() ===
        Position.getCastlingRookSquare(otherColor, true)
      ) {
        this.clearCastlingRight(otherColor, true);
      } else if (
        move.getCaptureSquare() ===
        Position.getCastlingRookSquare(otherColor, false)
      ) {
