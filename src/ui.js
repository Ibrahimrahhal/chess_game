...
export default class UIChessBoard {
	...
	static CHESSBOARD_PIECES_AND_SQUARES = this.CHESSBOARD_SQUARE + ", " + this.CHESSBOARD_PIECE;
	...
	static makeBoard() {
		var table = $("<table>");
		var filesRow = '<tr><th></th>' + "abcdefgh".split("").map(function(x) { return '<th class="file">' + x + "</th>"; }).join("") + "<th></th></tr>";
	...
	static clearMoving() {
		$(UIChessBoard.CHESSBOARD_PIECES_AND_SQUARES).removeClass("from to positional capture double-push en-passant promotion castle king-castle queen-castle");
	}
	...
	updatePieces() {
		...
		$(UIChessBoard.CHESSBOARD_SQUARE).removeClass("white black turn last-move " + Chess.PIECE_NAMES.join(" "));
	...
	updateMoves() {
		...
		$(UIChessBoard.CHESSBOARD_PIECE + ".can-move").mouseenter(function(event) {
			if (dragging) {
				return;
			}
			...
			fromElements.toggleClass("from", moves.some(function(move) { return move.getFrom() === from; }));
			...
			if (fromElements.hasClass("from")) {
				moves.forEach(function(move) {
					if (move.getFrom() === from) {
						...
						toElements.addClass(move.getKind() === Controller.Kind.POSITIONAL ? "positional" : "");
						toElements.addClass(move.isCapture() ? "capture" : "");
						toElements.addClass(move.getKind() === Controller.Kind.DOUBLE_PAWN_PUSH ? "double-push" : "");
						toElements.addClass(move.getKind() === Controller.Kind.EN_PASSANT_CAPTURE ? "en-passant" : "");
						toElements.addClass(move.isPromotion() ? "promotion" : "");
						toElements.addClass(move.isCastle() ? "castle" : "");
						toElements.addClass(move.getKind() === Controller.Kind.KING_CASTLE ? "king-castle" : "");
						toElements.addClass(move.getKind() === Controller.Kind.QUEEN_CASTLE ? "queen-castle" : "");
					}
				});
				...
				$(UIChessBoard.CHESSBOARD_SQUARE + ".to").droppable({
					"drop": function() {
						...
						var makeMoves = moves.filter(function(move) { return move.getFrom() === from && move.getTo() === to; });
